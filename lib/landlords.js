...
async function deleteLandlord( landlordId ) {
  ...
  // Check if landlord has any buildings
  const buildings = await all( "SELECT id FROM buildings WHERE landlord_id = ?", [landlordId] )
  if( 0 < buildings.length ) {
    throw new Error( "Cannot delete landlord with existing buildings. Please delete or reassign buildings first." )
  }
  ...
  return { success: true, deletedId: landlordId }
}
